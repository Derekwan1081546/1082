...
public:
		...
		: myData()
	{
		...
		//MapPtr rightptr = new value_type*[right.myData.mySize]();
		//pointer ptr = = new value_type[right.myData.mySize]();
		/*for (int i = 0; i < right.myData.mySize; i++)
		{
			push_back(rightptr[i]);
		}*/
		*this = right;
	}
	...
	deque& operator=(const deque& right)
	{
		if (&right != this) // avoid self-assignment
		{
			MapPtr newMap = new Ty * [right.myData.mapSize]();
			for (size_type i = right.myData.myOff / 4; i <= (right.myData.myOff + right.myData.mySize) / 4; i++)
			{
				size_type cur = i % right.myData.mapSize;
				newMap[cur] = new Ty[4]();
			}
			for (size_type cur = right.myData.myOff; cur < right.myData.myOff + right.myData.mySize; cur++)
			{
				if (cur >= right.myData.mapSize * 4)
				{
					newMap[(cur / 4) - right.myData.mapSize][cur % 4] = right.myData.map[(cur / 4) - right.myData.mapSize][cur % 4];
					continue;
				}
				newMap[cur / 4][cur % 4] = right.myData.map[cur / 4][cur % 4];
			}
			...
			resize(s);
			DequeIterator<DequeVal< Ty >> it1 = this->begin();
			DequeConstIterator<DequeVal< Ty >> it2 = right.begin();
...
bool operator==(const deque< Ty >& left, const deque< Ty >& right)
{
	if (left.size() != right.size())
		return false;
	DequeConstIterator<DequeVal< Ty >> rightptr = right.begin();
	DequeConstIterator<DequeVal< Ty >> leftptr = left.begin();
	for (; leftptr != left.end(); ++leftptr, ++rightptr)
		if (*leftptr != *rightptr)
			return false;
	return true;
}
